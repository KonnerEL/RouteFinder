...
def best_first_graph_search(problem, f):
    ...
    expandedNodes = 0
    i = 1
    graph["name"] = node.state
    graph["idParent"] = node.idParent
    graph["idNode"] = node.idNode
    graph["parent"] = node.idParent
    graph["children"] = []
    # graph[0]["children"][0]["name"] = "Florencia"
    # graph[0]["children"][0]["idParent"] = 0
    # graph[0]["children"][0]["idNode"] = 1
    # graph[0]["children"][0]["parent"] = "Leticia"
    # graph[0]["children"][0]["children"] = []
    # n = {}
    # n["name"] = "Mocoa"
    # n["idParent"] = 0
    # n["idNode"] = 2
    ...
    while frontier:
        node = frontier.pop() 
        print(node.state + " " + str(node.idParent))   
        if problem.goal_test(node.state):
            return [node, graph, expandedNodes, i]
        explored.add(node.state)
        if (len(node.expand(problem)) > 1):
            expandedNodes +=1
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
                frontier.append(child)
                ...
                if f(child) < f(incumbent):
                    ...
                    frontier.append(child)
            child.idParent = node.idNode
            child.idNode = i      
            searchNestedList(graph, child, node)
            i += 1 
    return None

def searchNestedList(graph,child,parent):
    if (graph["idNode"] == child.idParent):
        node = {}    
        node["name"] = child.state
        node["idParent"] = child.idParent
        node["idNode"] = child.idNode
        node["parent"] = parent.state
        node["children"] = []
        graph["children"].append(node)
        return    
    else:
        for sublist in graph["children"]:
            searchNestedList(sublist,child,parent)
